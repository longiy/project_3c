...
# Add to MovementComponent class variables:
var is_stopping: bool = false
var stop_deceleration: float = 15.0  # Faster than normal deceleration
...
func calculate_direct_movement(delta: float):
	...
	if is_stopping:
		var current_horizontal_speed = Vector2(character_core.velocity.x, character_core.velocity.z).length()
		if current_horizontal_speed > 0.1:
			# Smoothly decelerate in current movement direction
			var decel_factor = stop_deceleration * delta
			var new_velocity = Vector2(character_core.velocity.x, character_core.velocity.z)
			new_velocity = new_velocity.lerp(Vector2.ZERO, decel_factor)
			character_core.velocity.x = new_velocity.x
			character_core.velocity.z = new_velocity.y
		else:
			# Completely stopped
			character_core.velocity.x = 0
			character_core.velocity.z = 0
			is_stopping = false
			print("MovementComponent: Smooth stop complete")
		return
...
func finish_navigation():
	is_navigating = false
	has_rotation_target = false
	is_stopping = true  # Enable smooth stop
...
func initiate_smooth_stop():
	# Called by TargetControlComponent when drag ends
	is_navigating = false
	has_rotation_target = false
	is_stopping = true
	print("MovementComponent: Initiating smooth stop")
