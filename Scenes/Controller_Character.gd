...
@export var max_jumps = 5 ## Total jumps allowed (1 = single, 2 = double, etc.)
@export var rotation_speed = 10 ## How fast character rotates to face movement direction
...
func handle_rotation_and_movement(movement_vector: Vector3, delta: float, is_in_air: bool, sprinting: bool):
	...
	# Handle movement in air with momentum
	...
	if final_movement.length() > 0.1:
		...
		# Always rotate to face movement direction (no pure strafe exception)
		var target_rotation = atan2(final_movement.x, final_movement.z)
